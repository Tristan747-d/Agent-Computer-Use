...
description: 通过 dsh-computer-use:<tool> 工具（Hermes 的 server:tool 记法）读取和操作 macOS 本机 app 的界面（AX 树 + 截图 + 鼠标键盘）。当任务需要操作没有 CLI/API 的 GUI app 时使用。
...
本机自研的 DSH 插件，工具名一律为 `dsh-computer-use:<tool>`。
...
| 工具 | 只读 | 用途 |
|---|---|---|
| `dsh-computer-use:list_apps` | ✅ | 列出正在运行的正规 app |
| `dsh-computer-use:get_app_state` | ✅ | 取 app 关键窗口的 AX 树 + 截图（**不会抢前台**） |
| `dsh-computer-use:click` | | 按 element_index 或坐标点击 |
| `dsh-computer-use:set_value` | | 直接写 AXValue（表单首选） |
| `dsh-computer-use:select_text` | | 按文本匹配选中/定位光标 |
| `dsh-computer-use:press_key` | | xdotool 风格按键，如 `"Return"`、`"super+c"` |
| `dsh-computer-use:type_text` | | 输入文本（postToPid 直投，**不碰剪贴板**，支持中文） |
| `dsh-computer-use:scroll` | | 滚动 |
| `dsh-computer-use:drag` | | 拖拽 |
| `dsh-computer-use:perform_secondary_action` | | 执行 AX 树里列出的额外 action |
| `dsh-computer-use:clipboard_copy` | ✅ | 读 `AXSelectedText`（**不发 Cmd+C**，不动剪贴板） |
| `dsh-computer-use:start_live_view` | ✅ | 开启目标窗口**实时视频**（10fps MJPEG，不抢前台） |
| `dsh-computer-use:stop_live_view` | ✅ | 停止实时视频 |
| `dsh-computer-use:live_view_status` | ✅ | 查询实时视频状态与流地址 |
| `dsh-computer-use:probe_app` | ✅ | 先探测这个 app 是什么框架、多少节点、属于哪一层（**拉全树之前先调这个**） |
...
dsh-computer-use:get_app_state({ app: "TextEdit" })

// 2. 动作
dsh-computer-use:set_value({ app: "TextEdit", element_index: 2, value: "hello" })
